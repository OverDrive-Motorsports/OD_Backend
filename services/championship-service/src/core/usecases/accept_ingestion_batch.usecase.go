/**
	...
	##
	## accept_ingestion_batch.usecase.go - Package usecases source file for services/championship-service/src/core/usecases.
	##
*/
...
type AcceptIngestionBatchUseCase struct {
	serviceName string
	repository  ports.ChampionshipIngestionRepository
	now         func() time.Time
}

// NewAcceptIngestionBatchUseCase builds and returns a accept ingestion batch use case with its required dependencies.
func NewAcceptIngestionBatchUseCase(serviceName string, repository ports.ChampionshipIngestionRepository) *AcceptIngestionBatchUseCase {
